...
from nasap_net.yaml.lib import load_components, load_semi_light_assemblies
...
def load(yaml_str: str) -> dict[ID, Assembly]:
    ...
    return convert_semi_light_assemblies_to_rich_ones(
...
def _load_components_and_semi_light_assemblies(
        yaml_str: str,
) -> tuple[dict[str, Component], dict[ID, SemiLightAssembly]]:
