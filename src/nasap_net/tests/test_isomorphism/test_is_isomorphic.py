...
def test_different_component_kinds():
    ...
    X = Component(kind='X', sites=[0])
    MX = Assembly(
        components={'M1': M, 'X1': X}, bonds=[Bond('M1', 'X1', 0, 0)])

    FAKE_M = Component(kind='ANOTHER_KIND', sites=[0, 1])
    FAKE_MX = Assembly(
        components={'M1': FAKE_M, 'X1': X}, bonds=[Bond('M1', 'X1', 0, 0)])
